"""These codes are primarily from Smays Stem File of the same name adjusted by
    Marlesha Ellis and I(Austin Neal)"""
# region imports
...
class SpringLine(qtw.QGraphicsItem):
    def __init__(self, parent=None):
        super().__init__(parent)

    def boundingRect(self):
        return qtc.QRectF(-50, -50, 4, 10)

    def paint(self, painter, option, widget):
        painter.setPen(qtg.QPen(Qt.black))
        num_zigzags = 5
        height = 100  # Adjust the height to make the line longer
        width = 20
        step = height / (num_zigzags * 2)
        x_start = -50
        y_start = -50
        for i in range(num_zigzags):
            ...
            y2 = y1 + step
            if i % 2 == 0:
                painter.drawLine(qtc.QPointF(x_start, y1), qtc.QPointF(x_start + width, y2))
                x_start += width
            else:
                painter.drawLine(qtc.QPointF(x_start + width, y1), qtc.QPointF(x_start, y2))
                x_start -= width
